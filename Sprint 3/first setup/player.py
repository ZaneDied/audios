...
class PlayerWidget:
    def __init__(self, page: ft.Page):
        ...
        self.btn_play = ft.IconButton(icon=ft.Icons.PLAY_ARROW_ROUNDED, icon_size=32, icon_color="white", bgcolor="#6C5CE7", on_click=lambda _: self.player.play())
        self.btn_pause = ft.IconButton(icon=ft.Icons.PAUSE_ROUNDED, icon_size=26, icon_color="white", bgcolor="#2D2D3F", on_click=lambda _: self.player.pause())
        self.btn_stop = ft.IconButton(icon=ft.Icons.STOP_ROUNDED, icon_size=26, icon_color="white", bgcolor="#2D2D3F", on_click=self.stop)

        self.volume_slider = ft.Slider(
            min=0, max=100, value=70, width=120,
            active_color="#6C5CE7", inactive_color="#2D2D3F",
    ...
    def load_track(self, artist, album, song):
        """Loads and prepares a new song for playback."""
        self.player.stop()
        base_dir = os.path.join(os.getcwd(), "MusicLibrary", artist, album)
        ...
        self.track_artist.value = f"{artist} • {album}"
            ...
            if self.player.is_playing():
                ...
                if self.lrc_data:
                    for i in range(len(self.lrc_data)):
                        if self.lrc_data[i]["time"] <= pos:
                            if i == len(self.lrc_data) - 1 or pos < self.lrc_data[i+1]["time"]:
    ...
    def get_widget(self):
        return ft.Container(
            content=ft.Column([
                ...
                # Seek Bar & Timers (Matching PDF Page 3/4)
                ft.Column([
                ...
                ft.Row([
                    ft.Row([self.btn_pause, self.btn_play, self.btn_stop], spacing=10),
