...
class PlayerWidget:
    def __init__(self, page: ft.Page):
        ...
        # --- Lyric Sync Controls (Manual Text Entry) ---
    ...
    def on_offset_text_change(self, e):
        """Parse text input (e.g., '30', '-12', '2.5') dynamically into milliseconds."""
        val = e.control.value.strip()
        try:
            if val and val != "-":
                offset_sec = float(val)
                self.lyric_offset_ms = int(offset_sec * 1000)
                self.current_lyric_index[0] = -1  # Reset index to re-sync immediately
        except ValueError:
            pass  # Ignore invalid typing temporarily
